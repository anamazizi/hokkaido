...
4. **MIGRASI SQL IDEMPOTENT (supabase/migrations/fix_customer_reviews_rls.sql):**
   ...
   - Polisi yang diperbaiki:
     ...
     * `Allow staff/admin read all reviews` – pentadbir/staf boleh baca semua (termasuk pending).
     * `Allow staff/admin update reviews` – pentadbir boleh lulus/tarik balik kelulusan.
     * `Allow staff/admin delete reviews` – pentadbir boleh padam ulasan.
